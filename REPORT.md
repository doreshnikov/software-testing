...
* ✔️ написаны scenario-тесты на _Selenium_
    * рассмотрен простой сценарий взаимодействия с ожиданием ошибок
    * рассмотрен сложный сценарий взаимодействия
* ❌ за флоу запуска отвечает не _Selenoid_
    * в _GitHub Actions_ перед запуском тестов запускаются другие цели
    * локально можно настроить последовательное исполнение целей
    
---

```bash
cd server-jvm
./gradlew test -Pselenium
```
